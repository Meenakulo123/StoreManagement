...
import com.inn.super_market.dao.BillDao;
...
public class BillServiceImpl implements BillService {
	...
	public ResponseEntity<String> generateReport(Map<String, Object> requestMap) {
		...
		try {
			...
			if(validateRequestMap(requestMap)) {
				...
				PdfWriter.getInstance(document, new FileOutputStream(SuperConstants.STORE_LOCATION+"\\"+fileName+".pdf"));
				
				document.open();
				setRectangleInPdf(document);
				
				Paragraph chunk = new Paragraph("Edubridge Super Market",getFont("Header"));
				...
				JSONArray jsonArray = SuperUtils.getJsonArrayFromString((String)requestMap.get("productDetails"));
				for(int i=0;i<jsonArray.length();i++) {
					addRows(table,SuperUtils.getMapFromJson(jsonArray.getString(i)));
				}
				document.add(table);
			...
			return  SuperUtils.getResponseEntity("Request data not found",HttpStatus.OK);
	...
	public ResponseEntity<byte[]> getPdf(Map<String, Object> requestMap) {
		...
		try {
			...
			String filepath = SuperConstants.STORE_LOCATION+"\\"+(String)requestMap.get("uuid")+".pdf";
			if(SuperUtils.isFileExist(filepath)) {
	...
	public ResponseEntity<String> deleteBill(Integer id) {
		try {
			...
			if(!optional.isEmpty()) {
				...
				return SuperUtils.getResponseEntity("Bill deleted successfully",HttpStatus.OK);
 
				
			}
			return SuperUtils.getResponseEntity("Bill id does not exist",HttpStatus.OK);
